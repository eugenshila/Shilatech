...
export default function Warehouse(){
  ...
  if(auth==='login') return <Layout><section className="warehouseLoginPage"><div className="warehouseLoginShell"><div className="warehouseLoginBrand"><div className="warehouseCompanyMark">S</div><div><h1>SHILATECH</h1><span>AUTO SPARES · WAREHOUSE PORTAL</span></div></div><div className="warehouseLoginSelect"><b>Select a vehicle warehouse</b><span>Choose a logo, then sign in to view that brand's parts only.</span></div><div className="warehouseLoginLogos">{Object.entries(brandLogos).map(([name,src])=><button type="button" className={activeBrand===name?'selected':''} key={name} onClick={()=>setActiveBrand(activeBrand===name?'':name)} aria-pressed={activeBrand===name}><img src={src} alt={`${name} logo`}/><span>{name}</span><small>{activeBrand===name?'SELECTED':'OPEN'}</small></button>)}</div><form className="warehouseLoginCard" onSubmit={staffLogin}><span className="eyebrow">EMPLOYEE ACCESS ONLY</span><h2>{activeBrand?`${activeBrand} Warehouse Login`:'Warehouse Staff Login'}</h2><p>{activeBrand?`Sign in to open the ${activeBrand} parts warehouse.`:'Sign in with your authorized Shilatech employee account.'}</p>{error&&<div className="warehouseAlert">{error}</div>}<label>Email<input type="email" required autoComplete="username" value={login.email} onChange={e=>setLogin({...login,email:e.target.value})}/></label><label>Password<input type="password" required autoComplete="current-password" value={login.password} onChange={e=>setLogin({...login,password:e.target.value})}/></label><button className="button greenPrimary" disabled={busy}>{busy?'Signing in…':activeBrand?`Open ${activeBrand} warehouse`:'Sign in to warehouse'}</button><small>Customer accounts cannot access warehouse information.</small></form></div></section></Layout>;
