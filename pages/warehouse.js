...
export default function Warehouse(){
  ...
  if(auth==='login') return <Layout><section className="warehouseLoginPage"><div className="warehouseLoginShell"><div className="warehouseLoginBrand"><div className="warehouseCompanyMark">S</div><div><h1>SHILATECH</h1><span>AUTO SPARES · WAREHOUSE PORTAL</span></div></div><div className="warehouseLoginLogos">{Object.entries(brandLogos).map(([name,src])=><div key={name}><img src={src} alt={`${name} logo`}/><span>{name}</span></div>)}</div><form className="warehouseLoginCard" onSubmit={staffLogin}><span className="eyebrow">EMPLOYEE ACCESS ONLY</span><h2>Warehouse Staff Login</h2><p>Sign in with your authorized Shilatech employee account.</p>{error&&<div className="warehouseAlert">{error}</div>}<label>Email<input type="email" required autoComplete="username" value={login.email} onChange={e=>setLogin({...login,email:e.target.value})}/></label><label>Password<input type="password" required autoComplete="current-password" value={login.password} onChange={e=>setLogin({...login,password:e.target.value})}/></label><button className="button greenPrimary" disabled={busy}>{busy?'Signing in…':'Sign in to warehouse'}</button><small>Customer accounts cannot access warehouse information.</small></form></div></section></Layout>;

  return <Layout>
    <section className="pageHero compactHero"><div className="container warehouseHeroRow"><div><span className="eyebrow">PRIVATE STAFF PORTAL</span><h1>Warehouse & 3PL Operations</h1><p>Customer orders, FIFO inventory, brand-separated storage, barcodes, preorders and returns.</p></div><button className="warehouseLogout" onClick={logout}>Sign out</button></div></section>
    <section className="section warehousePage"><div className="container">
      ...
      {data&&<>
        ...
        <div className="warehousePanel warehouseOrderQueue"><div className="warehousePanelHead"><div><span className="eyebrow">ONLINE SALES → WAREHOUSE</span><h2>Customer order fulfilment queue</h2><p>Process every website order from picking through dispatch. Barcode scans are checked before FIFO stock is issued.</p></div></div>
          {data.warehouseOrders?.length?<div className="warehouseOrderGrid">{data.warehouseOrders.map(o=><div className="warehouseOrderCard" key={o.id}>
        ...
        <div className="brandStorageGrid">{(data.brandSummary||[]).map(area=><button type="button" className={`brandStorageCard brandStorageButton${activeBrand===area.brand_code?' active':''}`} key={area.id} onClick={()=>setActiveBrand(activeBrand===area.brand_code?'':area.brand_code)} aria-pressed={activeBrand===area.brand_code}><div className="brandStorageLogo">{brandLogos[area.brand_code]?<img src={brandLogos[area.brand_code]} alt={`${area.brand_code} logo`}/>:<strong>{area.brand_code}</strong>}</div><div className="brandStorageBody"><span>{area.code} ZONE</span><h3>{area.name}</h3><p>{area.units_on_hand} units · {area.active_batches} active FIFO batches</p><small>{activeBrand===area.brand_code?'SHOWING THIS WAREHOUSE · CLICK TO VIEW ALL':'OPEN WAREHOUSE →'}</small></div></button>)}</div>
        {activeBrand&&<div className="storageAssignment"><b>{activeBrand} warehouse</b><span>Showing only {activeBrand} parts and FIFO batches.</span><button type="button" onClick={()=>setActiveBrand('')}>View all brands</button></div>}
        ...
        <div className="warehouseForms">
          <form className="warehousePanel" onSubmit={submitReceipt}>
            ...
            {receiptMode==='existing'?<label>Part<select required value={receipt.productId} onChange={e=>selectReceiptProduct(e.target.value)}><option value="">Select part</option>{visibleProducts.map(p=><option value={p.id} key={p.id}>{p.brand} · {p.part_no} — {p.name}</option>)}</select></label>:<div className="newPartFields">
              ...
              <label>Vehicle brand<select required value={newPart.brand} onChange={e=>setNewPartBrand(e.target.value)}>{brands.map(b=><option key={b}>{b}</option>)}</select></label>
