...
function normalise(payload){const found=[];const seen=new Set();function walk(value,depth=0){if(!value||depth>7)return;if(Array.isArray(value)){value.forEach(item=>walk(item,depth+1));return;}if(typeof value!=='object')return;const partNo=scalar(value,['articlenumber','articleno','partnumber','partno','oenumber','articleoemno']);const id=scalar(value,['articleid','id']);const name=scalar(value,['description','productname','articlename','genericarticledescription','productgroup']);if(partNo&&(name||id)){const key=partNo.toUpperCase();if(!seen.has(key)){seen.add(key);found.push({id:id||key,partNo,name:name||'Compatible automotive part',brand:scalar(value,['brandname','suppliername','brand','manufacturername'])||'Catalogue supplier',imageUrl:nestedScalar(value,['imageurl','image','pictureurl','mediaurl','url'])});}}Object.values(value).forEach(item=>walk(item,depth+1));}walk(payload);return found.slice(0,24);}
export default async function handler(req,res){
 ...
 try{
  ...
  const payload=await response.json(),parts=normalise(payload);return res.json({parts,source:'Auto Parts Catalog',...(parts.length?{}:{diagnostic:responseShape(payload)})});
