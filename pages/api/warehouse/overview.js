...
export default async function handler(req, res) {
  ...
  try {
    const [metrics, batches, preorders, returns, products, warehouses, brandSummary] = await Promise.all([
      query(`SELECT
      ...
      query(`SELECT b.id,b.batch_no,b.received_qty,b.available_qty,b.received_at,b.supplier_name,
        p.id AS product_id,p.part_no,p.name,p.brand,p.barcode,w.name AS warehouse,w.code AS warehouse_code,w.brand_code,wb.code AS bin_code
        ...
        ORDER BY p.brand,b.received_at ASC LIMIT 150`),
      ...
      query(`SELECT r.id,r.return_no,r.quantity,r.reason,r.defect_type,r.disposition,r.status,r.created_at,p.part_no,p.name,p.brand
      ...
      query(`SELECT w.id,w.code,w.name,w.brand_code,
        COALESCE(SUM(b.available_qty),0)::int AS units_on_hand,
        COUNT(b.id) FILTER (WHERE b.available_qty>0)::int AS active_batches
        FROM warehouses w LEFT JOIN inventory_batches b ON b.warehouse_id=w.id
        WHERE w.active=TRUE AND w.storage_type='BRAND'
        GROUP BY w.id,w.code,w.name,w.brand_code ORDER BY w.name`)
    ]);
    res.status(200).json({ metrics: metrics.rows[0], batches: batches.rows, preorders: preorders.rows, returns: returns.rows, products: products.rows, warehouses: warehouses.rows, brandSummary: brandSummary.rows });
  } catch (error) {
    console.error(error);
