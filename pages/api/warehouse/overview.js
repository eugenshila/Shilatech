...
export default async function handler(req,res){
  ...
  try{
    const [metrics,batches,preorders,returns,products,warehouses,brandSummary,warehouseOrders]=await Promise.all([
      ...
      query(`SELECT pr.id,pr.quantity,pr.allocated_qty,pr.expected_at,pr.status,pr.customer_name,p.part_no,p.name,p.brand FROM preorders pr JOIN products p ON p.id=pr.product_id WHERE pr.status NOT IN ('FULFILLED','CANCELLED') ORDER BY pr.created_at ASC LIMIT 50`),
