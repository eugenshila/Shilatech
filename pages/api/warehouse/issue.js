...
export default async function handler(req,res){
  ...
  if(referenceType==='ORDER'||referenceType==='ONLINE_ORDER')return res.status(400).json({error:'Use the order picking workflow for online orders.'});
  ...
  try{
    ...
    const reserved=await client.query(`SELECT COALESCE(SUM(GREATEST(wi.quantity-wi.picked_qty,0)),0) qty FROM warehouse_order_items wi JOIN warehouse_orders wo ON wo.id=wi.warehouse_order_id WHERE wi.product_id=$1 AND wi.storage_area_id=$2 AND wo.status<>'CANCELLED'`,[productId,warehouseId]);
    ...
    if(available-Number(reserved.rows[0].qty)<quantity||Number(product.rows[0].stock)<quantity)throw new Error('Insufficient unreserved stock. Pending online orders must be protected.');
