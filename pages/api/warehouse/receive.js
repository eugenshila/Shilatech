...
export default async function handler(req, res) {
  ...
  if (!Number.isInteger(productId) || !Number.isInteger(warehouseId) || !Number.isInteger(quantity) || quantity <= 0 || !batchNo) {
    return res.status(400).json({ error: 'Product, storage area, batch number and positive quantity are required.' });
  }
  ...
  try {
    ...
    if (w.brand_code !== p.brand) {
      throw new Error(`${p.brand} parts must be received into the ${p.brand} storage area. This prevents brands from being mixed.`);
    }
    ...
    await client.query(`INSERT INTO warehouse_audit (employee_id,action,entity_type,entity_id,details)
      VALUES ($1,'RECEIVE_STOCK','inventory_batch',$2,$3::jsonb)`, [Number(session.sub),String(batch.rows[0].id),JSON.stringify({ productId, brand:p.brand, quantity, batchNo, warehouseId, warehouseCode:w.code, binCode })]);

    await client.query('COMMIT');
    res.status(201).json({ batch: batch.rows[0], barcode: p.barcode || p.part_no, storageArea: w });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error(error);
    if (error.code === '23505') return res.status(409).json({ error: 'That batch number already exists for this product and storage area.' });
