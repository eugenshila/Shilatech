...
export default async function handler(req,res){
  ...
  try{
    ...
    res.status(200).send(`<!doctype html><html><head><meta charset="utf-8"><title>${title} ${esc(j.order_no)}</title><style>body{font-family:Arial,sans-serif;color:#111;margin:28px}header{border-bottom:3px solid #58b72a;padding-bottom:14px;margin-bottom:18px}.brandLogo{display:block;width:230px;height:auto;margin-bottom:10px}.brand{font-size:24px;font-weight:900;color:#3f9120}.sub{font-size:11px;letter-spacing:2px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:8px 22px;margin:16px 0;font-size:13px}.box{border:1px solid #bbb;padding:12px;margin:14px 0}table{width:100%;border-collapse:collapse;margin-top:16px;font-size:12px}th,td{border:1px solid #bbb;padding:8px;text-align:left}th{background:#f1f1f1}.sign{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:55px}.line{border-top:1px solid #333;padding-top:6px;font-size:11px}@media print{button{display:none}body{margin:12mm}}</style></head><body><button onclick="window.print()">Print</button><header><img class="brandLogo" src="/shilatech-logo-small.webp" alt="Shilatech Auto Spares"></img><div class="sub">WAREHOUSE & 3PL OPERATIONS</div><h1>${title}</h1></header><div class="meta"><div><b>Order:</b> ${esc(j.order_no)}</div><div><b>Warehouse job:</b> ${esc(j.job_no)}</div><div><b>Customer:</b> ${esc(j.customer_name)}</div><div><b>Phone:</b> ${esc(j.phone)}</div><div><b>Delivery:</b> ${esc(j.delivery_zone)}</div><div><b>Status:</b> ${esc(j.status)}</div></div><div class="box"><b>Delivery address</b><br>${esc(j.delivery_address)}<br><br><b>Payment</b>: ${esc(j.payment_method)} · ${esc(j.payment_status)} · KSh ${Number(j.total_kes||0).toLocaleString()}</div><table><thead><tr><th>#</th><th>Brand</th><th>Part</th><th>Storage area</th><th>Qty</th><th>Picked</th></tr></thead><tbody>${rows}</tbody></table><div class="sign"><div class="line">Prepared / Picked by</div><div class="line">Checked / Authorized by</div></div></body></html>`);
