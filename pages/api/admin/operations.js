...
export default async function handler(req,res){
 const s=readSession(req); if(!s||!allowed.has(s.role))return res.status(403).json({error:'Management access required.'});
 if(req.method==='GET')try{
  const [suppliers,pos,low,metrics]=await Promise.all([
   ...
   query(`SELECT (SELECT count(*) FROM products WHERE active=TRUE) products,(SELECT COALESCE(sum(stock),0) FROM products WHERE active=TRUE) units,(SELECT count(*) FROM orders WHERE created_at>=date_trunc('month',now())) month_orders,(SELECT COALESCE(sum(total_kes),0) FROM orders WHERE payment_status='Paid' AND created_at>=date_trunc('month',now())) month_paid_sales,(SELECT count(*) FROM returns WHERE status NOT IN ('CLOSED','RESOLVED')) open_returns`)
  ]);return res.json({suppliers:suppliers.rows,purchaseOrders:pos.rows,lowStock:low.rows,metrics:metrics.rows[0]});
