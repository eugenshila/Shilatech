...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const p = cleanProduct(req.body);
      const result = await query(`UPDATE products SET slug=$1,name=$2,brand=$3,category=$4,part_no=$5,part_type=$6,price_kes=$7,years=$9,models=$10::jsonb,engine=$11,active=$12,updated_at=NOW() WHERE id=$13 AND stock=$8 AND price_kes=$7 AND (brand=$3 OR NOT EXISTS(SELECT 1 FROM inventory_batches WHERE product_id=$13)) RETURNING *`, [p.slug,p.name,p.brand,p.category,p.partNo,p.partType,p.price,p.stock,p.years,JSON.stringify(p.models),p.engine,p.active,id]);
      if (!result.rowCount) return res.status(409).json({ error: 'Reload this product. Existing prices require Requests & approvals. Stock must be changed through receiving or approved adjustments; the brand of a batched part cannot be changed.' });
