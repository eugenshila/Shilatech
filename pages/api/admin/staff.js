...
const roles=new Set(['warehouse_manager','warehouse_clerk','picker','packer','dispatch','finance','auditor','delivery_driver']);
...
export default async function handler(req,res){
 ...
 const b=req.body||{};const action=String(b.action||'').toUpperCase();const c=await getPool().connect();try{await c.query('BEGIN');
  if(action==='CREATE'){
   ...
   const hash=await bcrypt.hash(password,12);const r=await c.query(`INSERT INTO customers(name,email,phone,password_hash,role) VALUES($1,$2,$3,$4,$5) RETURNING id,name,email,phone,role`,[name,email,phone,hash,role]);
  ...
  if(action==='SET_ROLE'){
   ...
   const r=await c.query(`UPDATE customers SET role=$1 WHERE id=$2 AND role<>'admin' RETURNING id,name,email,phone,role`,[role,id]);if(!r.rowCount)throw new Error('Employee not found or administrator role cannot be changed here.');
