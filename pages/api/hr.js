...
import {setupRecords,recordsReady} from '../../lib/hr-records.mjs';
export default async function handler(req,res){
 ...
 try{
  ...
  if(req.method==='GET'){
   ...
   return res.json({userId:s.id,role:s.role,ready:Boolean(ready.rows[0].leaves),recordsReady:await recordsReady(c),leaves:leaves.rows,payslips:payslips.rows,staff:staff.rows,events:events.rows});
  }
  const b=req.body||{};
  await c.query('BEGIN');transaction=true;
  if(b.action==='SETUP'){
   await setupRecords(c,s);
  }else if(b.action==='LEAVE'){
   if(!await recordsReady(c))throw Error('Ask HR to enable the new review workflow first.');
  ...
  }else if(b.action==='DECIDE'){if(!await recordsReady(c))throw Error('Ask HR to enable the new review workflow first.');await reviewLeave(c,s,b);}
