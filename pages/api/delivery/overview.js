...
export default async function handler(req,res){
  ...
  try{
    const driverFilter=session.role==='delivery_driver'?'AND (dj.driver_id=$1 OR dj.driver_id IS NULL)':'';
    ...
    const result=await query(`SELECT dj.id,dj.status,dj.driver_id,dj.recipient_name,dj.signed_at,dj.delivered_at,
      ...
      GROUP BY dj.id,wo.id,o.id ORDER BY dj.created_at ASC LIMIT 100`,params);
