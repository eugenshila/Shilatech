...
export function signSession(user) {
  return jwt.sign({ sub: String(user.id), email: user.email, role: user.role || 'customer', name: user.name, pv:user.password_version||0 }, secret(), { expiresIn: '7d' });
}
...
export async function readSession(req) {
  ...
  try {
    ...
    const result=await query('SELECT id,name,email,role,location_id,password_version,must_change_password FROM customers WHERE id=$1',[token.sub]);
    const user=result.rows[0];
    if(!user||Number(token.pv||0)!==user.password_version)return null;
    if(user.must_change_password&&!['/api/auth/me','/api/auth/password'].includes(String(req.url||'').split('?')[0]))return null;
