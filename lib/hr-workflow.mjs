...
export function leaveDecision(actor,leave,decision,note){
 ...
 if(actor.role==='hr'&&decision==='NOTE'&&['PENDING_MANAGER','PENDING_ADMIN'].includes(leave.status))return leave.status;
 const expected=actor.role==='general_manager'?'PENDING_MANAGER':actor.role==='admin'?'PENDING_ADMIN':null;
 ...
 if(actor.role==='admin'&&(!leave.manager_id||String(leave.manager_id)===String(actor.id)))throw Error('A separate general manager must review first.');
 return decision==='REJECT'?'REJECTED':actor.role==='general_manager'?'PENDING_ADMIN':'APPROVED';
}
...
export async function reviewLeave(c,actor,body){
 ...
 if(body.decision!=='NOTE')await c.query(`UPDATE hr_leave_requests SET status=$1,manager_id=CASE WHEN $2='general_manager' THEN $3 ELSE manager_id END,admin_id=CASE WHEN $2='admin' THEN $3 ELSE admin_id END,review_note=$4,updated_at=NOW() WHERE id=$5`,[status,actor.role,actor.id,body.note.trim(),body.id]);
