...
export async function readLedger(c,s,period){
 ...
 if(posExists){const q=await c.query("SELECT COALESCE(SUM(total_kes),0)::bigint AS revenue FROM counter_sales WHERE to_char(created_at AT TIME ZONE 'Africa/Nairobi','YYYY-MM')=$1",[period]);pnl.posRevenueCents=Number(q.rows[0].revenue)*100;const allocationsExist=Boolean((await c.query("SELECT to_regclass('public.counter_sale_allocations') AS t")).rows[0].t);if(allocationsExist){const ct=await c.query("SELECT COALESCE(SUM(a.quantity*a.unit_cost_kes),0)::bigint AS cogs FROM counter_sale_allocations a JOIN counter_sale_items i ON i.id=a.sale_item_id JOIN counter_sales s ON s.id=i.sale_id WHERE to_char(s.created_at AT TIME ZONE 'Africa/Nairobi','YYYY-MM')=$1",[period]);pnl.cogsCents=Number(ct.rows[0].cogs)*100;}}
